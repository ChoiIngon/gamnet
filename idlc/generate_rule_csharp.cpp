...
bool GenerateRuleCSharp::CompileMessage(const Token::Message* pToken)
{
	...
	if ("" != pToken->m_sMessageNo)
	{
		if("" != pToken->m_sParentName)
		{
			std::cout << "\t" << "public new const int MSG_ID = " << pToken->m_sMessageNo << ";" << std::endl;
		}
		else
		{
			std::cout << "\t" << "public const int MSG_ID = " << pToken->m_sMessageNo << ";" << std::endl;
		}
	}
	...
	for (const auto& itr : pToken->list_)
	{
		if (Token::TYPE_VARDECL == itr->Type())
		{
			...
			std::cout << "\t" << "public " << TranslateVariableType(pVarDecl->m_pVarType) << "\t" << pVarDecl->m_pVarName->GetName() << " = " << GenerateVariableInit(pVarDecl->m_pVarType) << ";" << std::endl;
		}
