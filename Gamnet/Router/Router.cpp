...
void Listen(const char* service_name, int port, int max_session)
{
	...
	Log::Write(GAMNET_INF, "Gamnet router start(service_name:", service_name, ", local ip:", Network::GetLocalAddress().to_string(), ", port:", port, ", capacity:", max_session, ", id:", Singleton<RouterListener>::GetInstance().localAddr_.id, ")");
}
