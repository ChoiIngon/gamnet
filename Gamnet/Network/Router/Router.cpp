...
void Listen(const char* service_name, int port, const std::function<void(const Address& addr)>& onAccept, const std::function<void(const Address& addr)>& onClose)
{
	...
	LOG(INF, "Gamnet router start(service_name:", service_name, ", local ip:", Network::Tcp::GetLocalAddress().to_string(), ", port:", port, ", id:", Singleton<LinkManager>::GetInstance().localAddr_.id, ")");
}
