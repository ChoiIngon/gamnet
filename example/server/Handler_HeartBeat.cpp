...
void Handler_HeartBeat::Recv_Ntf(std::shared_ptr<Session> session, std::shared_ptr<Gamnet::Network::Packet> packet)
{
	...
	try {
		...
		{
			throw Gamnet::Exception(ERROR_INVALID_MSG_FORMAT, "message load fail");
		}

		if(NULL == session->user_data)
		{
			throw Gamnet::Exception(ERROR_INVALID_USER, "invalid user");
		}
		...
		{
			throw Gamnet::Exception(ERROR_INCORRECT_DATA, "lost message seq:", session->user_data->msg_seq);
		}
